...
class TestVcs extends TestBase
{
	...
	public function new(id:VcsID, vcsExecutable:String, url:String, ?rev:String) {
		super();
		this.id = id;
		this.url = url;
		this.rev = rev;
	...
	public function testCloneBranch():Void {
		final vcs = createVcs();
		final dir = id.getName() + counter++;
		vcs.clone(dir, {url: url, branch: "develop"});
	...
	public function testCloneBranchCommit():Void {
		final vcs = createVcs();
		final dir = id.getName() + counter++;
		vcs.clone(dir, {url: url, branch: "develop", commit: rev});
	...
	public function testUpdateBranchTag_0_9_2():Void {
		final vcs = createVcs();
	...
	public function testUpdateRev():Void {
		final vcs = createVcs();
	...
	public function testUpdateBranch__afterUserChanges_withReset():Void {
		final vcs = createVcs();
		final dir = id.getName() + counter;
